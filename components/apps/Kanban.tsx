...
const Kanban: React.FC = () => {
  ...
  const [columns, setColumns] = useState<KanbanColumn[]>([
    {
      ...
      title: 'To Do',
      tasks: [
        { id: 't1', title: 'Research competitor analysis', tag: 'Strategy', tagColor: 'bg-blue-100 text-blue-700', assignees: ['u1'], comments: 2, attachments: 1 },
        { id: 't2', title: 'Draft user interview questions', tag: 'Research', tagColor: 'bg-purple-100 text-purple-700', assignees: ['u2'], comments: 0, attachments: 0 },
      ]
    },
    {
      id: 'progress',
      title: 'In Progress',
      tasks: [
        { id: 't3', title: 'Design system update v2', tag: 'Design', tagColor: 'bg-pink-100 text-pink-700', assignees: ['u1', 'u3'], comments: 5, attachments: 3 },
      ]
    },
    {
      id: 'review',
      title: 'Review',
      tasks: [
        { id: 't4', title: 'Mobile app accessibility audit', tag: 'Dev', tagColor: 'bg-amber-100 text-amber-700', assignees: ['u4'], comments: 1, attachments: 0 },
      ]
    },
    {
      id: 'done',
      title: 'Done',
      tasks: [
        { id: 't5', title: 'Q1 Budget Finalization', tag: 'Finance', tagColor: 'bg-green-100 text-green-700', assignees: ['u2'], comments: 8, attachments: 2 },
      ]
    }
  ]);
  ...
  const users = [
      { id: 'u1', name: 'Sarah Miller', avatar: 'https://i.pravatar.cc/150?u=sarah' },
    ...
    <div className="h-full flex flex-col bg-gray-50/50 relative overflow-hidden">
      ...
      <div className="bg-white/80 backdrop-blur-md border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div>
           <div className="flex items-center gap-3 mb-1">
             ...
             <div className="flex -space-x-2">
                {users.slice(0, 3).map(u => (
                    <div key={u.id} className="w-8 h-8 rounded-full border-2 border-white bg-gray-200 flex items-center justify-center overflow-hidden">
           ...
           <p className="text-xs text-gray-500 font-medium">Oct 24 - Nov 7 • 12 days remaining</p>
        </div>
            ...
            {columns.map((column) => (
                <div 
                    ...
                    className={`w-80 flex flex-col h-full max-h-full rounded-2xl transition-colors duration-300 ${
                        dragOverColumnId === column.id ? 'bg-brand-50/50 ring-2 ring-brand-200' : ''
                    }`}
                ...
                >
                    <div className="flex items-center justify-between mb-4 px-1">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="px-2 py-0.5 bg-gray-200 text-gray-600 text-xs rounded-full font-medium">{column.tasks.length}</span>
                        </div>
                        ...
                        {column.tasks.map((task) => (
                            <div 
                                ...
                                className={`bg-white p-4 rounded-xl border shadow-sm transition-all cursor-grab active:cursor-grabbing group relative ${
                                    draggedTaskId === task.id ? 'opacity-50 rotate-3 scale-95 shadow-xl border-brand-300 z-20' : 'border-gray-200 hover:shadow-md hover:border-brand-300'
                                }`}
                            >
                                <div className="flex justify-between items-start mb-2">
                                    <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide ${task.tagColor}`}>
                                        {task.tag}
                                    </span>
                                ...
                                <h4 className="font-semibold text-gray-800 text-sm mb-3 leading-snug">{task.title}</h4>
                                        ...
                                        {task.assignees.map((aId, i) => {
                                            ...
                                            return (
                                                <div key={i} className="w-6 h-6 rounded-full border-2 border-white bg-gray-100 flex items-center justify-center overflow-hidden">
                                                    {u ? <img src={u.avatar} className="w-full h-full object-cover" /> : <User size={10} className="text-gray-500" />}
                                                </div>
                                            )
                                        })}
                                    </div>
      ...
      >
          ...
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0 z-10">
              <div className="flex items-center gap-3">
                  <div className="px-3 py-1 bg-gray-100 rounded text-xs font-mono text-gray-500">PROJ-124</div>
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-4">
                          ...
                          <div className="relative">
                              ...
                              >
                                  <User size={14} />
                                  <span>{selectedAssignees.length > 0 ? `${selectedAssignees.length} Assignees` : 'Assign'}</span>
                              </button>
                                      ...
                                      <div className="space-y-1">
                                          {users.map(user => (
                                              ...
                                              >
                                                  ...
                                                  {selectedAssignees.includes(user.id) && <Check size={14} className="text-brand-600" />}
                                              </button>
                                          ))}
                                      </div>
                                  </div>
                  ...
                  <div>
                      ...
                      <div className="border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-brand-500/20 transition-all">
                          <div className="bg-gray-50 border-b border-gray-200 px-3 py-2 flex items-center gap-1">
                              <button className="p-1.5 hover:bg-gray-200 rounded text-gray-500"><Bold size={14} /></button>
                              <button className="p-1.5 hover:bg-gray-200 rounded text-gray-500"><Italic size={14} /></button>
                              <button className="p-1.5 hover:bg-gray-200 rounded text-gray-500"><List size={14} /></button>
                              <div className="w-px h-4 bg-gray-300 mx-1"></div>
                              <button className="p-1.5 hover:bg-gray-200 rounded text-gray-500"><Link size={14} /></button>
                              <button className="p-1.5 hover:bg-gray-200 rounded text-gray-500"><ImageIcon size={14} /></button>
                          </div>
                          <textarea 
                            rows={6} 
                  ...
                  <div className="grid grid-cols-2 gap-8">
                      ...
                      <div>
                          ...
                          <div className="flex flex-wrap gap-2 mb-2">
                              {tags.map((tag, i) => (
                                  <span key={i} className="px-2 py-1 bg-gray-100 text-gray-600 rounded text-xs font-medium flex items-center gap-1 group">
                                      {tag}
                                      <button onClick={() => removeTag(tag)} className="opacity-0 group-hover:opacity-100 hover:text-red-500"><X size={10} /></button>
                                  </span>
                              ))}
                              <input 
                                type="text" 
                      ...
                      <div>
                          ...
                          <div className="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center hover:bg-gray-50 transition-colors cursor-pointer">
                              <p className="text-xs text-gray-500">Drag files here or <span className="text-brand-600 font-medium">browse</span></p>
                          </div>
                      </div>
                  </div>
                              ...
                              <div className="flex-1">
                                  <div className="bg-gray-50 rounded-xl rounded-tl-none p-3 border border-gray-100">
                                      <div className="flex items-center justify-between mb-1">
                                          <span className="text-xs font-bold text-gray-700">Alexandra T.</span>
                                          <span className="text-[10px] text-gray-400">2h ago</span>
                                      </div>
                                      <p className="text-sm text-gray-600">Initial requirements gathered. Waiting for design review.</p>
                                  </div>
                              </div>
                          </div>
                              ...
                              <div className="flex-1">
                                   <div className="relative">
                                       ...
                                       <div className="absolute right-2 bottom-2 flex gap-1">
                                           <button className="p-1 text-gray-400 hover:text-gray-600"><Smile size={16} /></button>
                                           <button className="p-1 text-brand-600 hover:text-brand-700"><Check size={16} /></button>
                                       </div>
                                   </div>
                              </div>
                          </div>
